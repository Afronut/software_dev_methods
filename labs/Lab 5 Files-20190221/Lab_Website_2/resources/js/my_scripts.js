...
*/
function loadStatsPaget() {
	let win = 0;
	let lose = 0;
	let stat_table = document.getElementById("stats_table");
	for (let i = 2; i < stat_table.rows.length; i++) {
		score_home = parseInt(stat_table.rows[i].cells[2].innerHTML);
		score_opp = parseInt(stat_table.rows[i].cells[3].innerHTML);
		if ((score_home - score_opp) > 0) {
			stat_table.rows[i].cells[4].innerHTML = "Buff";
			win++;
		} else {
			stat_table.rows[i].cells[4].innerHTML = stat_table.rows[i].cells[1].innerHTML;
			lose++;
		}
	}
...
function loadPlayersPage() {
	myDrop = document.getElementById("player_selector");
	for (var key in players) {
		anchor = document.createElement('a');
		anchor.setAttribute('href', '#');
		if (key == 0) {
			anchor.setAttribute("onclick", "switchPlayers(0)");
		} else if (key == 1) {
			anchor.setAttribute("onclick", "switchPlayers(1)");
		} else if (key == 2) {
			anchor.setAttribute("onclick", "switchPlayers(2)");
		} else if (key == 3) {
...
function switchPlayers(playerNum) {
	...
	document.getElementById('avg_p_yards').innerHTML = Math.floor(players[playerNum].pass_yards * 100 / players[playerNum].games_played) / 100;
	document.getElementById('r_yards').innerHTML = players[playerNum].rushing_yards;
	document.getElementById('avg_r_yards').innerHTML = Math.floor(players[playerNum].rushing_yards * 100 / players[playerNum].games_played) / 100;
	document.getElementById('rec_yards').innerHTML = players[playerNum].receiving_yards;
	document.getElementById('avg_rec_yards').innerHTML = Math.floor(players[playerNum].receiving_yards * 100 / players[playerNum].games_played) / 100;
